...
export default function RoutinesClient({ initialRoutines, initialLogs, journeys, labels, migrationMissing }) {
  ...
  async function deleteRoutine(routine) {
    if (!window.confirm('Excluir esta rotina? O histórico também será removido.')) return;
    const response = await fetch('/api/routines/delete', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ routine_id: routine.id }) });
    if (response.ok) { setRoutines((old) => old.filter((item) => item.id !== routine.id)); setPauseId(null); }
  }
  function beginEdit(routine) { setEditing(routine); setEditForm({ name: routine.name, ideal_text: routine.ideal_text, minimum_text: routine.minimum_text || '', privacy: routine.privacy || 'private' }); setPauseId(null); }
  async function saveEdit() {
    if (!editing || !editForm?.name.trim() || !editForm.ideal_text.trim()) return;
    setBusy(true); setError('');
    try {
      const response = await fetch('/api/routines/edit', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ routine_id: editing.id, ...editForm }) });
      const data = await response.json();
      if (!response.ok) { setError(labels.error); return; }
      setRoutines((old) => old.map((item) => item.id === editing.id ? data.routine : item));
  ...
  return (<div className="routine-shell">
    ...
    {editing && editForm && <div className="routine-dialog-backdrop" role="presentation"><section className="routine-dialog" role="dialog" aria-modal="true" aria-labelledby="routine-edit-title"><div className="routine-dialog-top"><div><p className="eyebrow">{labels.options}</p><h2 id="routine-edit-title">Editar rotina</h2></div><button className="icon-close" onClick={() => setEditing(null)} aria-label={labels.close}>×</button></div><div className="routine-form"><label>{labels.nameQ}<input maxLength={120} value={editForm.name} onChange={(e) => setEditForm((v) => ({ ...v, name: e.target.value }))} /></label><label>{labels.idealQ}<textarea maxLength={240} rows={3} value={editForm.ideal_text} onChange={(e) => setEditForm((v) => ({ ...v, ideal_text: e.target.value }))} /></label><label>{labels.minimumQ}<textarea maxLength={240} rows={3} value={editForm.minimum_text} onChange={(e) => setEditForm((v) => ({ ...v, minimum_text: e.target.value }))} /></label><label>{labels.privacyQ}<select value={editForm.privacy} onChange={(e) => setEditForm((v) => ({ ...v, privacy: e.target.value }))}><option value="private">{labels.private}</option><option value="milestones">{labels.milestones}</option><option value="profile">{labels.profile}</option></select></label></div><div className="routine-dialog-actions"><button className="ghost-btn" onClick={() => setEditing(null)}>{labels.back}</button><button className="cta" disabled={busy} onClick={saveEdit}>{busy ? '…' : 'Salvar'}</button></div></section></div>}
  </div>);
}

function RoutineCard({ routine, logs, labels, journeyTitle, busy, pauseId, setPauseId, onLog, onPause, onResume, onArchive, onDelete, onEdit }) {
  const today = localDate();
