...
export default function DiarioClient({ labels }) {
  ...
  async function askUp() {
    if (!upOn || !text.trim() || busy) return;
    setBusy(true); setUpText('');
    try {
      const r = await fetch('/api/diary-up', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: text.trim() }) });
      const data = await r.json();
      if (r.ok && data.text) {
        setUpText(data.text);
        const list = entries.map((e) => e.date === date ? { ...e, up: data.text } : e);
