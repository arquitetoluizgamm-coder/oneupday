...
function Media({ photo, video, href, labels, caption, onRatio, children, below, alt = '', hasTrack = false }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting && entry.intersectionRatio >= 0.58) start();
      else stop();
    }, { threshold: [0, 0.35, 0.58, 0.85] });
  ...
  const conteudo = video ? (
    <video
      ref={videoRef}
      src={comCapa(video)} controls playsInline preload="metadata" autoPlay muted={videoMuted}
...
function MidiaGaleria({ item, labels }) {
  ...
  const hasMedia = !!item.url;

  return (
    <>
      {item.routine && (
        <section className="feed-routine-card" aria-label={item.routine.label}>
          <span>{item.routine.label}</span>
          <h3>{item.routine.name}</h3>
          <div className="feed-routine-versions">
            <p><small>{item.routine.ideal}</small><b>{item.routine.ideal_text}</b></p>
            {item.routine.minimum_text && <p><small>{item.routine.minimum}</small><b>{item.routine.minimum_text}</b></p>}
          </div>
        </section>
      )}
