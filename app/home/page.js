...
async function ensureProfile(supabase, user) {
  const { data: existing } = await supabase
    .from('profiles').select('id, name, handle').eq('id', user.id).maybeSingle();
  ...
  const profile = {
    id: user.id,
    name: meta.full_name || meta.name || base,
    handle,
...
export default async function Home() {
  ...
  const { data: journeys } = await supabase
    .from('journeys').select('*').eq('owner_id', user.id)
  ...
  if (list.length) {
    const { data: stats } = await supabase
      .from('journey_stats').select('*')
      .in('journey_id', list.map(j => j.id));
      ...
      <main className="wrap">
        <section className="home-head">
          <div>
            ...
            <h1>What moved forward today?</h1>
          </div>
          <a className="cta" href="/new">+ New journey</a>
        </section>

        {list.length === 0 && (
          <div className="empty">
            <b>No journey yet.</b>
            <p>Start one in under a minute. Pick something real and post the first step.</p>
            <a className="cta" href="/new">Create your first journey</a>
          </div>
        )}
          ...
          return (
            <section className="jcard" key={j.id}>
              <div className="jcard-head">
                <div>
                  <h2>{j.title}</h2>
                  <span>Day {day} of {j.total_days} · {s.streak || 0} day streak</span>
                </div>
                <a className="view-link" href={`/${j.slug}`} target="_blank" rel="noreferrer">View public ↗</a>
              </div>
              <div className="bar"><span style={{ width: pct + '%' }} /></div>
              <Composer journeyId={j.id} nextDay={day + 1} />
            </section>
          );
        })}
      </main>
