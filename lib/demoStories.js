import { getHistoria } from './historias';
const DEMO_OWNERS = [
...
export function getDemoStory(slug, locale = 'pt') {
  const demo = buildDemoStories(locale).find((story) => story.slug === slug);
  if (demo) return demo;
  // as historias de 7 dias usam a mesma pagina de jornada
  return getHistoria(slug, locale);
}

const DEMO_MOODS = { 'joana-volta-estudar': 'down', 'helena-dormir-cedo': 'tired', 'marina-run-again': 'happy', 'amira-sair-de-casa': 'anxious', 'paulo-cem-dias': 'motivated', 'diego-reabrir-oficina': 'grateful' };
export function buildDemoFeedItems(locale = 'pt') {
  return buildDemoStories(locale).map((story) => {
    ...
    const prevU = story.updates[story.updates.length - 2];
    const gap = prevU ? (latest.day_number - prevU.day_number) : 0;
    story = { ...story, demoMood: DEMO_MOODS[story.slug] || null };
    return {
      id: `feed-${story.slug}`,
      ...
      journey: {
        ...
        current_day: story.stats.current_day,
        progress_pct: story.stats.progress_pct,
      },
      owner: {
        ...story.owner,
        id: null,
        ...
        mood: story.demoMood || null,
      },
      supporters: DEMO_OWNERS
        .filter((o) => o.name !== story.owner.name)
        .map((o) => ({ name: o.name, handle: o.handle, avatar_url: o.avatarUrl, avatar_color: o.avatarColor })),
    };
  });
}
