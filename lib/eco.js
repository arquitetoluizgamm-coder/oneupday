...
export function fraseBase(fato, locale = 'pt') {
  ...
  const es = {
    promessa: `Ayer dejaste este paso decidido. Hoy volviste con el resultado.`,
    retorno: `Pasaron ${d.dias} días sin registrar esta jornada y hoy volviste. El capítulo continúa desde aquí.`,
    meta: `Este es el día ${d.total} de esta jornada. Los días no fueron iguales, pero la historia llegó hasta aquí.`,
    marco: `Este es el día ${d.dia} de esta jornada. Un buen lugar para mirar atrás.`,
    primeiro: `Este es el comienzo registrado de esta jornada. Un día volverás aquí y compararás.`,
    seguiu: `Ayer fue un día difícil y hoy volviste a registrar.`,
    capitulo: `Este es el capítulo ${d.n} de esta jornada.`,
  };
  const tab = locale === 'en' ? en : locale === 'es' ? es : pt;
  return tab[fato?.tipo] || '';
}

// ============================================================
// A PERGUNTA — e por que ela NÃO é escrita pela IA
//
// O Eco só fala quando ninguém falou: ele espera 45 minutos e
// desiste se houver qualquer comentário. Ou seja, a fala dele
// nunca disputa espaço com a comunidade — ela ocupa o silêncio.
//
// Nesse lugar, uma observação encerra o assunto e uma pergunta
// devolve a palavra para quem escreveu. É o único mecanismo do
// app que produz MAIS texto humano; todo o resto (resumo,
// retrospectiva, marcador) reorganiza o que já existe.
//
// Mas a pergunta é escolhida AQUI, pelo mesmo motivo que o fato
// é: uma pergunta inventada por modelo é uma pergunta que
// ninguém revisou, feita para alguém em dia ruim. A IA continua
// proibida de perguntar — a regra na instrução fica como está.
// Ela redige a observação; a pergunta vem pronta daqui e é
// colada depois, palavra por palavra.
//
// Elas perguntam sobre o que a pessoa FEZ, nunca sobre o que
// ela sente. "O que ajudou hoje?" é um convite a contar. "Como
// você está se sentindo?" é consultório, e este app não é.
//
// Dois fatos ficam SEM pergunta, de propósito:
//
//   retorno — quem sumiu dez dias e voltou não precisa
//     explicar o sumiço. Pergunta ali é cobrança com roupa
//     de curiosidade.
//
//   seguiu — o dia seguinte a um dia difícil. A frase que já
//     existe ("ontem foi difícil e você registrou de novo
//     hoje") é melhor calada que interrogativa.
//
// E o chamador ainda cala a pergunta quando o próprio post é
// um dia difícil, qualquer que seja o fato.
// ============================================================
export function perguntaBase(fato, locale = 'pt') {
  const pt = {
    promessa: 'O que fez esse passo caber no dia de hoje?',
    meta: 'Se você começasse hoje, o que faria diferente?',
    marco: 'O que é diferente hoje, comparado ao primeiro dia?',
    primeiro: 'Como você vai perceber que avançou?',
    capitulo: 'O que ajudou hoje?',
    retorno: '',
    seguiu: '',
  };
  const en = {
    promessa: 'What made that step fit into your day?',
    meta: 'If you started today, what would you do differently?',
    marco: 'What is different today, compared to day one?',
    primeiro: 'How will you know you moved forward?',
    capitulo: 'What helped today?',
    retorno: '',
    seguiu: '',
  };
  const es = {
    promessa: '¿Qué hizo que este paso cupiera en tu día de hoy?',
    meta: 'Si empezaras hoy, ¿qué harías diferente?',
    marco: '¿Qué es diferente hoy comparado con el primer día?',
    primeiro: '¿Cómo sabrás que avanzaste?',
    capitulo: '¿Qué te ayudó hoy?',
    retorno: '',
    seguiu: '',
  };
  const tab = locale === 'en' ? en : locale === 'es' ? es : pt;
...
//
// A regra "sem pergunta no fim" continua valendo e é essencial:
// a pergunta existe, mas vem de perguntaBase() acima, revisada,
// e é colada depois que esta resposta é validada.
// ------------------------------------------------------------
...
- Sem pergunta, em nenhum lugar da frase. A pergunta do Eco é escrita fora daqui e colada depois desta observação.
