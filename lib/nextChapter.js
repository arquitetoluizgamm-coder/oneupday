...
export async function computeNextChapter(supabase, userId, primary, t) {
  const out = { mode: null, line: null, lead: '', identity: '', env: null, sealedEnv: false, ctaHref: '/perfil' };
  ...
  try {
    ...
    try {
      const { data: env } = await supabase.from('envelopes').select('id, text, journey_id, created_at, opened_at').eq('user_id', userId).order('created_at', { ascending: false }).limit(1).maybeSingle();
      if (env) {
        const isToday = dayKey(new Date(env.created_at).getTime()) === dayKey(now);
        if (out.mode === 'sealed' && isToday) out.sealedEnv = true;
        else if ((out.mode === 'reveal' || out.mode === 'return') && !env.opened_at && !isToday) {
          out.env = { id: env.id, text: env.text };
          if (env.journey_id === primary.id && primary.slug) out.ctaHref = `/perfil/jornada/${primary.slug}`;
        }
      }
    } catch {}
  } catch {}
